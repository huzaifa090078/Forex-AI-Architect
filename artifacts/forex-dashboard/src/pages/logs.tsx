...
export default function LogsPage() {
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="flex-1 min-h-0 bg-[#050505] border-border/50 shadow-2xl flex flex-col overflow-hidden rounded-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/30 bg-[#0a0a0a]">
        ...
        <div className="flex-1 overflow-auto p-5 custom-scrollbar bg-[linear-gradient(to_bottom,#ffffff02_1px,transparent_1px)] bg-[size:100%_24px]">
          {isLoading ? (
            <div className="space-y-3">
              {Array.from({length: 15}).map((_, i) => <Skeleton key={i} className="h-5 w-full bg-white/5 rounded-none" />)}
            </div>
          ) : logsData?.items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground/30 font-mono text-sm gap-2">
            ...
            <div className="font-mono text-[11px] leading-[24px] tracking-tight">
              {logsData?.items.map(log => (
                <div key={log.id} className="flex flex-col sm:flex-row sm:gap-4 hover:bg-white/5 px-2 -mx-2 rounded-sm transition-colors border-l-2 border-transparent hover:border-white/20">
                  <span className="text-muted-foreground/40 whitespace-nowrap shrink-0 selection:bg-primary selection:text-primary-foreground">
                    {new Date(log.createdAt).toISOString().replace('T', ' ').substring(0, 23)}
                  </span>
                  <span className={cn("uppercase font-bold w-[80px] shrink-0", 
                    log.level === "error" || log.level === "critical" ? "text-red-500 shadow-red-500/20" :
                    log.level === "warning" ? "text-amber-500" :
                    log.level === "info" ? "text-blue-400" : "text-gray-500"
                  )}>
                    [{log.level}]
                  </span>
                  <span className="text-primary/70 w-[140px] shrink-0 truncate font-semibold">
                    {log.module}
                  </span>
                  <span className={cn("break-words selection:bg-primary selection:text-primary-foreground", 
                     log.level === "error" || log.level === "critical" ? "text-red-400 font-medium" : "text-gray-300"
                  )}>
                    {log.message}
                  </span>
                </div>
